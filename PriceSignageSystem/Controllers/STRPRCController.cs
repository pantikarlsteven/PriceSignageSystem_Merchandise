        ...
        public FileResult ExportDataTableToExcel(string tab, DateTime date, string filter, string deptFilter)
        {
            ...
            using (XLWorkbook workbook = new XLWorkbook())
            {
                ...
                for (int i = 0; i < dataTable.Columns.Count; i++)
                {
                    ...
                    var columnName = "";
                    if (dataTable.Columns[i].ColumnName == dataTable.Columns[i].ColumnName.ToUpper())
                        columnName = dataTable.Columns[i].ColumnName;
                    else
                    {
                        columnName = Regex.Replace(dataTable.Columns[i].ColumnName, "(\\B[A-Z])", " $1");
                        if (columnName == "Correct Price")
                            columnName = "Correct Price (For OPS)";
                ...
                using (var memoryStream = new System.IO.MemoryStream())
                {
                    ...
                    var fileName = "Exemptions_" + DateTime.Today.ToShortDateString() + ".xlsx";
